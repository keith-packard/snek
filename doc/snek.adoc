...
displays along with user input. User input is shown *`in a bold face, like
this`* on the lines which start with `>` or `+`, Snek output is shown
`in a normal face, like this` on other lines.
...
to include single quote marks in them. Snek always prints strings
using single quotes, so the output here is the same as before.
