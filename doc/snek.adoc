...
construct programs from elements manipulated with a pointing device
can become frustratingly slow to create code. These languages don't
develop portable skills to bring to more advanced languages.
Sophisticated languages like C, Java and even Python are so large as
to overwhelm the novice with rich semantics like “objects” and other
...
elsewhere. However, while the smallest of these machines are similar
...
I used screw terminals for all of the inputs and outputs, stuck a
battery pack on the back and included four high-current H-bridge motor
controllers to help animate the robots. They're still Arduino's though,
there's an ATmega 328P processor and a FTDI USB to serial converter,
so we were able to use the stock Arduino development tools.

However, students struggled with the complex syntax, especially typing
the obscure punctuation marks and remembering to insert semicolons. I
...
experimenting a bit slower than other systems. There have been
students who got past the obstacles and figured out how to use them:
...
The hardware was just what we wanted, a few students were using the
